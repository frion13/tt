...
Путь с пробелами заключай в кавычки. `--install-links` устанавливает копию локального пакета в node_modules: импорты runner и reporter разрешаются через зависимости целевого проекта даже для скилла из личного каталога. Если @playwright/test уже установлен в совместимой версии, убери его из команды, сохрани существующую версию и lock-файл. Если package.json отсутствует, сначала создай его через `npm init -y`. Папка `playwright-qa` содержит единственный общий runtime `playwright-qa-skills`. После обновления скилла повтори установку из его папки.
